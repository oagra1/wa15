...
<script>
import { permissionCheck, redeemCode, getLicenseStatus } from '@/api/permission'
...
export default {
  ...
  methods: {
    ...
    async checkActiveCode() {
      ...
      const normalizedNoHyphen = original.replace(/-/g, '')
      if (this.checkSubkey(normalized)) {
        if (MIGRATION_SIMPLE_FLOW) {
          try {
            ...
            const candidates = Array.from(new Set([original, normalizedNoHyphen]))
            let ok = false
            let data = null
            let status = 0
