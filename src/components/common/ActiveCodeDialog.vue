    ...
    <div class="codeInput">
      ...
      <div class="tip-div">
        <svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="18" height="18">
          <path
            fill="red"
            d="M512 128c212.064 0 384 171.936 384 384s-171.936 384-384 384S128 724.064 128 512 299.936 128 512 128z m0 64C335.264 192 192 335.264 192 512s143.264 320 320 320 320-143.264 320-320S688.736 192 512 192z m32 480v64h-64v-64h64z m0-384v320h-64V288h64z"
          ></path>
        </svg>
        <el-tooltip
      ...
      <div
        @click="openPricing"
        style="display:flex;align-items:center;margin-left:25px;padding:10px 0;cursor:pointer;"
      >
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="18" height="18">
          <g>
            <path
              ...
              fill="rgba(142,139,139,1)"
            ></path>
          </g>
        </svg>
        <div style="font-size:12px;margin-left:4px;color:#8e8b8b">
          How can I obtain an activation code?
        </div>
      </div>

      <div
        @click="openFaq"
        style="display:flex;align-items:center;margin-left:25px;text-align:left;width:350px;cursor:pointer;"
      >
        <div>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="18" height="18">
            <g>
...
export default {
  ...
  methods: {
    ...
    async checkActiveCode() {
      ...
      // Se a migração simples estiver ativa, usar Supabase e pular regras legadas
      if (MIGRATION_SIMPLE_FLOW) {
        try {
          // identifiers opcionais; não bloquear se ausentes
          const { userPhoneNum, userEmail } =
            (await new Promise((resolve) => {
              chrome.storage?.local?.get?.(['userPhoneNum', 'userEmail'], resolve)
            }).catch(() => ({}))) || {}

          const { ok, data } = await redeemCode({
            code: original,
          ...
          await chrome.storage?.local?.set?.({
            [STORAGE_LICENSE_KEY]: data, // status, expires_at, etc.
            [STORAGE_ACTIVATION_FLAG]: true,
            paid_mark: true // compat com UI antiga
          })
      ...
      // ===== LEGADO (fallback quando MIGRATION_SIMPLE_FLOW === false) =====
    ...
    checkActiveNum() {
      this.languageVal = ''
      if (MIGRATION_SIMPLE_FLOW) {
        // No fluxo novo, não aplicamos validação de 8 caracteres
        return true
      }
      // Legado: exige 8 chars (apenas para compat)
    ...
    checkSubkey(activeCode) {
      if (MIGRATION_SIMPLE_FLOW) {
        // No fluxo novo, ignoramos o checksum legado
        return true
      }
      // Legado: verificação de checksum
      let firstActiveCodeStr = activeCode.substring(0, 1)
      let secondActiveCodeStr = activeCode.substring(1, activeCode.length)
      let totalCount = 0
      for (let ch of secondActiveCodeStr) {
        totalCount += ch.charCodeAt(0)
      }
      let hexTotalCountStr = totalCount.toString(16)
      let endHexStr = hexTotalCountStr.substring(hexTotalCountStr.length - 1, hexTotalCountStr.length)
      return endHexStr === firstActiveCodeStr
    },
...
.tip-div > div { font-size:12px; margin-left:4px; color:red; display:-webkit-box; overflow:hidden; -webkit-box-orient:vertical; -webkit-line-clamp:2; word-wrap:break-word; width:calc(100% - 18px); word-break:keep-all; }
...
.el-dialog__footer .el-button--success, .el-dialog__footer .el-button--success:hover {
  width:160px; height:32px; border-radius:4px; background-color:#37b64a; font-size:14px; text-align:center;
  font-family:Roboto; align-items:center; justify-content:center; padding:0;
}
</style>
<style>
